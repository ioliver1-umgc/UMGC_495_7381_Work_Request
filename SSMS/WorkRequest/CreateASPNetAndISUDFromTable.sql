...
SET @ENTITY_ID_VARIABLE = 'WorkRequest_ID';
...
SET @ENTITY_TABLE_OR_VIEW_NAME = '''WRWorkRequests''';
        ...
        BEGIN
            ...
            ELSE IF @VBNET_TYPE = 'STRING'
                ...
                BEGIN
                    ...
                    PRINT '             _' + @COLUMN_NAME + '= oStringCleaner.MustBeProperCharacters(sVal,'
                          + CONVERT(VARCHAR, @LEN) + ')';
                    PRINT '        Else';
                    PRINT '             _' + @COLUMN_NAME + '= ""';
                END;
...
BEGIN
    ...
    PRINT '	oSqlCom.Parameters.Add("@' + @ENTITY_ID_VARIABLE + '", SqlDbType.VarChar, 0).Value = '
          + @ENTITY_ID_VARIABLE + '.ToString';
    PRINT 'oReader = ' + @DATABASE_HANDLER_INSTANTIATED_INSTANCE_NAME + '.' + @DATABASE_UPDATE_HANDLER_PROCEDURE
          + '(oSqlCom)''--(oSqlCom, [Optional] ServerName,[Optional] Databasename)';
    PRINT '';
    ...
    SET NOCOUNT ON;
    DECLARE MyCursorII INSENSITIVE CURSOR FOR
    SELECT DISTINCT
           [COLUMN_NAME],
           [LENGTH],
           [VBNET_TYPE]
    ...
    DECLARE MyCursorInit INSENSITIVE CURSOR FOR
    SELECT DISTINCT
           [COLUMN_NAME],
           [LENGTH],
           UPPER([SQL_TYPE])
...
BEGIN
    ...
    DECLARE MyCursorIV INSENSITIVE CURSOR FOR
    SELECT DISTINCT
           [COLUMN_NAME],
           [LENGTH],
           [VBNET_TYPE],
    ...
    BEGIN
        ...
        BEGIN
            IF @VBNET_TYPE = 'String'
            BEGIN
                PRINT '	oSqlCom.Parameters.Add("@' + @COLUMN_NAME + '", SqlDbType.VarChar, ' + CONVERT(VARCHAR, @LEN)
                      + ').Value = oData.' + @COLUMN_NAME + '.ToString';
            END;
            ELSE
            BEGIN
                PRINT '	oSqlCom.Parameters.Add("@' + @COLUMN_NAME + '", SqlDbType.' + @SQL_TYPE + ').Value = oData.'
                      + @COLUMN_NAME;
            END;
        END;
        FETCH NEXT FROM MyCursorIV
    ...
    PRINT '			oReader = ' + @DATABASE_HANDLER_INSTANTIATED_INSTANCE_NAME + '.'
          + @DATABASE_UPDATE_HANDLER_PROCEDURE
          + '(oSqlCom)''--(oSqlCom, [Optional] ServerName,[Optional] Databasename)';
...
BEGIN
    ...
    PRINT '    Public Function ' + @DATABASE_UPDATE_HANDLER_PROCEDURE
          + '(ByVal inSqlCom As SqlCommand) As SqlDataReader';
    PRINT '	';
    PRINT '	';
...
BEGIN
    ...
    DECLARE MyCursorV INSENSITIVE CURSOR FOR
    SELECT DISTINCT
           [COLUMN_NAME],
           [LENGTH],
           [VBNET_TYPE],
...
-- ============================================='
    ...
    DECLARE MyCursorVI INSENSITIVE CURSOR FOR
    SELECT DISTINCT
           [TABLE_NAME],
           [COLUMN_NAME]
    FROM zzTemp_ClassCreator
    ...
    PRINT 'BEGIN';
    DECLARE MyCursorVII INSENSITIVE CURSOR FOR
    SELECT DISTINCT
           [TABLE_NAME],
           [COLUMN_NAME]
    FROM zzTemp_ClassCreator
    ...
    DECLARE MyCursorVIII INSENSITIVE CURSOR FOR
    SELECT DISTINCT
           [TABLE_NAME],
           [COLUMN_NAME]
    FROM zzTemp_ClassCreator
	...
	*/
    ...
    PRINT '';
    DECLARE MyCursorVIIII INSENSITIVE CURSOR FOR
    SELECT DISTINCT
           [TABLE_NAME],
           [COLUMN_NAME],
           [SQL_TYPE]
    ...
    DECLARE MyCursorX INSENSITIVE CURSOR FOR
    SELECT DISTINCT
           [TABLE_NAME],
           [COLUMN_NAME],
           [SQL_TYPE]
