...
<GENERAL_INFO>
		...
		<FILENAME>
			UPDBPROETL1.DBO.ADMIN.CREATE_COLUMNS_TO_CLASS_PROPERTIES
		</FILENAME>
		<APPNAME>
			NA
		</APPNAME>
		...
		<SERVER>UPDB-PRO-LOADER1</SERVER>
		<CALLEDBY NAME="" TYPE="" />
		<CALLS NAME="" TYPE="" />
		<DESC>
			...
			NAME OF THE SPROCS TO PERFORM THESE ACTIONS IS GENERATED IN THIS CODE, THE ACTUAL WRITING OF THE SPROCS 
			IS LEFT TO THE PROGRAMMER. 
				
			
			1. ANSWER QUESTIONS 1-11 IN SECTION --***SET THESE VALUES BEFORE RUNNING THIS QUERY***--
...
IF EXISTS 
(SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[dbo].[zzTemp_ClassCreator]') 
AND OBJECTPROPERTY(id, N'IsTable') = 1) DROP TABLE [dbo].[zzTemp_ClassCreator] 
GO
...
--***SET THESE VALUES BEFORE RUNNING THIS QUERY***--
	...
	SET @ENTITY_ID_VARIABLE = 'strIdPki'
	...
	SET @ENTITY_TABLE_OR_VIEW_NAME= '''TCM_USERS'''
	...
	--FOLLOWING OPTIONAL SETTINGS CAN GENERALLY BE LEFT AT THEIR DEFAULT VALUES.
...
IF @CREATE_PROPERTIES = 1 OR @CREATE_ENTIRE_CLASS = 1
BEGIN
...
'
	...
	DECLARE MyCursor INSENSITIVE CURSOR
		FOR SELECT DISTINCT [COLUMN_NAME], [LENGTH],[VBNET_TYPE],[STRING_CHECKING]
	...
	BEGIN
		...
		END
		FETCH NEXT FROM MyCursor INTO @COLUMN_NAME,@LEN,@VBNET_TYPE,@STRING_CHECKING
	END exitsub: DEALLOCATE MyCursor
	SET NOCOUNT OFF
END
IF @CREATE_ENTIRE_CLASS=0
...
IF @CREATE_INITIALIZATION = 1 OR @CREATE_ENTIRE_CLASS = 1
BEGIN
...
END
	...
	print 'oReader = ' + @DATABASE_HANDLER_INSTANTIATED_INSTANCE_NAME + '.' 
	+ @DATABASE_UPDATE_HANDLER_PROCEDURE + '(oSqlCom)''--(oSqlCom, [Optional] ServerName,[Optional] Databasename)'
	PRINT ''
	...
	DECLARE MyCursorII INSENSITIVE CURSOR
		FOR SELECT DISTINCT [COLUMN_NAME], [LENGTH],[VBNET_TYPE]
			FROM zzTemp_ClassCreator
			ORDER BY COLUMN_NAME
	OPEN MyCursorII
	...
	BEGIN
		...
		BEGIN
	 	...
	 	END
		FETCH NEXT FROM MyCursorII INTO @COLUMN_NAME,@LEN,@VBNET_TYPE
	END exitsubII: DEALLOCATE MyCursorII
	...
	PRINT '		Me.DATA_FOUND = "N"'
		...
		FOR SELECT DISTINCT [COLUMN_NAME], [LENGTH],UPPER([SQL_TYPE])
			FROM zzTemp_ClassCreator
			ORDER BY COLUMN_NAME
	OPEN MyCursorInit
	...
	BEGIN
		...
		BEGIN
			...
			ELSE IF @SQL_TYPE IN ('INT','TINYINT','NUMERIC','REAL','SMALLINT')
				PRINT '		ME.' +  @COLUMN_NAME + '= 0'
			ELSE IF @SQL_TYPE IN ('DATETIME')
...
IF @CREATE_CALL_PARAMATERIZEDUPDATE_PROCEDURE_FROM_CLASS = 1 OR @CREATE_ENTIRE_CLASS = 1
	BEGIN	
...
END
	...
	DECLARE MyCursorIV INSENSITIVE CURSOR
		FOR SELECT DISTINCT [COLUMN_NAME], [LENGTH],[VBNET_TYPE],[SQL_TYPE]
			FROM zzTemp_ClassCreator
			ORDER BY COLUMN_NAME
	OPEN MyCursorIV
...
PRINT ''
PRINT ''
PRINT '''**************************START UPDATE SPROC CREATION******************************************************************************************'
	...
	BEGIN
	  IF (@@fetch_Status <> -2) 
		  ...
		  IF @VBNET_TYPE='String'
		    BEGIN
			PRINT '	@' + @COLUMN_NAME + ' VARCHAR(' + CONVERT(VARCHAR,@LEN) + '),'
		    END
		  ELSE
		    BEGIN
	...
	<GENERAL_INFO>
		...
		<DESC>
			PROVIDES METHOD FOR UPDATING ENTITY DATA 
		</DESC>		
		<!--LIST OF CHANGES SUBMITTED-->
		<CHANGE BY="" DATE="">
			<DESC></DESC>
	...
	*/
	...
	DECLARE MyCursorVI INSENSITIVE CURSOR
		...
		FOR SELECT  DISTINCT [TABLE_NAME],[COLUMN_NAME]
			FROM zzTemp_ClassCreator
	...
	PRINT 'GRANT  EXECUTE  ON [dbo].' + @SPROC_FOR_UPDATES + '  TO [WEB_APPUSER]'
	PRINT 'GO'
	PRINT ''
...
PRINT '''**************************START GET DATA SPROC CREATION******************************************************************************************'
	...
	*/
	...
	DECLARE MyCursorVIIII INSENSITIVE CURSOR
		...
		FOR SELECT  DISTINCT [TABLE_NAME],[COLUMN_NAME],[SQL_TYPE]
			FROM zzTemp_ClassCreator
			ORDER BY [TABLE_NAME],[COLUMN_NAME]
	...
	PRINT 'WHERE [PUT IDENTITY COLUMNNAME HERE] = @' + @ENTITY_ID_VARIABLE
	PRINT 'END'
	PRINT 'ELSE'
	PRINT 'BEGIN'
	PRINT ''
...
END
	...
	PRINT ''
	PRINT ''
	PRINT 'GRANT  EXECUTE  ON [dbo].' + @SPROC_FOR_GETTNG_ENTITY_DATA + '  TO [WEB_APPUSER]'
	PRINT 'GO'
	PRINT ''
	PRINT 'SET QUOTED_IDENTIFIER OFF' 
	PRINT 'GO'
