...
DECLARE @ENTITY_TABLE_ID NVARCHAR(255)
...
SET @ENTITY_TABLE_OR_VIEW_NAME = '''tblSTUDENTS'''
--10a. Set the ID of the table using the ordinal position
SET @ENTITY_TABLE_ID = 
(select column_name from information_schema.columns where table_name =   REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','')  
...
-- sp_xml_preparedocument block
-- 1. Remove comma after the last columns of the SELECT and Common Table Expression(CTE) in sp_xml_preparedocument block

-- INSERT fixes:
-- 2. Remove SQL_SYSTEM_USER,LAST_UPDATE_DATE, BY_WHO as appropriate from ROW node in the test data, SELECT, and INSERT statments. These are 
-- statically added after looping through all the columns in the table.
-- 3. Remove all computed columns from the INSERT/SELECT statements and test XML
-- 4. Remove the TABLE_ID Column from the INSERT/SELECT statements
-- 5. Set correct dates/non nulls in test data. Remove all other bugs I wasn''t smart enough TO find

-- UPDATE fixes:
-- 6. Remove comma after the last column in the SET column list.
-- 7. PROD.LAST_UPDATE_DATE = GETDATE(), SQL_SYSTEM_USER = SYSTEM_USER, BY_WHO= @BY_WHO
-- 8. Remove the update to the TABLE_ID, remove any computed column references
-- Final Fix: Remove these instructions from this section
-- ============================================='
    END    
	...
	PRINT '			DECLARE @SYS_AUTHENTICATION_AUTHORIZATION_RESULT INT'
    PRINT '			SET @SYS_AUTHENTICATION_AUTHORIZATION_RESULT = dbo.udf_GET_SYS_AUTHENTICATION_AUTHORIZATION_RESULT(@BY_WHO,0) --Does the user have permission to ' + @SPROC_FOR_UPDATES
    PRINT 'IF @SYS_AUTHENTICATION_AUTHORIZATION_RESULT = 1'
    PRINT 'BEGIN --SYS_AUTHENTICATION_AUTHORIZATION_RESULT = 1'
    PRINT ''
    PRINT '		  --Custom "Per Transaction Variables" here'
    PRINT ''
    PRINT '		  --Use sp_xml_preparedocument to shred XML to relational table. This method is more fogiving that the STUFF method as it allows the procedure to run if no nodes are passed. '
	PRINT '			DECLARE @doc_handle INT --Create integer to reference the XML Document'
	PRINT '			EXEC sp_xml_preparedocument @doc_handle OUTPUT, @XML_DATA'
	PRINT '			SELECT ACTION, '
    ...
    END 
    DEALLOCATE MyCursorV
    PRINT '		INTO #' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','')
	PRINT '		FROM OPENXML(@doc_handle, ''/rows/row''' + ', 2)'
    PRINT '		WITH ( ACTION NVARCHAR(10) ''action''' +','
	DECLARE MyCursorV INSENSITIVE CURSOR
    FOR
    SELECT DISTINCT
            [COLUMN_NAME],
    ...
    BEGIN
        ...
        BEGIN
            IF @VBNET_TYPE = 'String'
            BEGIN
                PRINT '			' + @COLUMN_NAME + ' NVARCHAR(' + CONVERT(VARCHAR,@LEN) + ')' + ' ''' + LOWER(@COLUMN_NAME) + '''' +','
            END
            ELSE
            BEGIN
                PRINT '			' + @COLUMN_NAME + ' ' + UPPER(@SQL_TYPE) + ' ''' + LOWER(@COLUMN_NAME) + '''' +','
            END
        END
        FETCH NEXT FROM MyCursorV INTO @COLUMN_NAME,@LEN,@VBNET_TYPE,@SQL_TYPE
    END 
    DEALLOCATE MyCursorV
	PRINT '			)'
	PRINT	'		EXEC sp_xml_removedocument @doc_handle --Don''t forget to remove the document handele to recover resources'
    PRINT'			/*If needed add "Per Transaction" Columns here
    		--Add the CATEGORY "Per Transaction COLUMN
			See uspCOMMENTS_CUD for example          
			--Update temp table to reflect the CATEGORY Passed
			See uspCOMMENTS_CUD for example*/'            
	PRINT'			--Check to see if there are any insert actions in the temporary table. If ' + @ENTITY_TABLE_ID + '  = 0, it is an INSERT'            
	PRINT'            IF EXISTS ( SELECT  ' + @ENTITY_TABLE_ID + '  FROM    #' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','') + ' WHERE '+ @ENTITY_TABLE_ID +'  = 0 )'
	PRINT'            BEGIN --INSERT'
	PRINT'			PRINT ''INSERTING' + ' ' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','') + ''''
...
DECLARE MyCursorV INSENSITIVE CURSOR
    ...
    PRINT'				WHERE TEMP.' + @ENTITY_TABLE_ID + '  = 0'
    PRINT'			END --INSERT'    
    PRINT''
    PRINT'			--Since the passed ' + @ENTITY_TABLE_ID + '  is NOT 0, it must be an update or a delete action '
	PRINT'			IF EXISTS ( SELECT  ' + @ENTITY_TABLE_ID + '  FROM    #' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','') + ' WHERE ACTION=''UPDATE'' )'
	PRINT'			BEGIN --Update'
	PRINT'			PRINT ''UPDATING' + ' ' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','') + ''''   
	PRINT'			UPDATE PROD'
	PRINT'			SET '
	DECLARE MyCursorV INSENSITIVE CURSOR
    FOR
    ...
    DEALLOCATE MyCursorV  
    PRINT'		FROM ' + '#' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','') + ' AS TEMP'
    PRINT'			INNER JOIN ' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','') + ' AS PROD ON PROD.' + @ENTITY_TABLE_ID + '  = TEMP.' + @ENTITY_TABLE_ID + '  AND TEMP.ACTION = ''UPDATE'''
    PRINT'		END'
	PRINT'		      --Check to see if there are any delete actions in the temporary table. '          
	PRINT'            IF EXISTS ( SELECT  ' + @ENTITY_TABLE_ID + '  FROM    #' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','') + ' WHERE ACTION = ''DELETE''' +')'
	PRINT'				BEGIN --DELETE'
	PRINT'					PRINT ''DELETING' + ' ' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','') + ''''
	PRINT'					DELETE FROM ' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','')  
	PRINT'					WHERE	' + @ENTITY_TABLE_ID + '  IN (SELECT ' + @ENTITY_TABLE_ID + '  FROM #' + REPLACE(@ENTITY_TABLE_OR_VIEW_NAME,'''','') + ' WHERE ACTION = ''DELETE''' +')'
	PRINT'				END'					
	PRINT'END --SYS_AUTHENTICATION_AUTHORIZATION_RESULT = 1'
	PRINT'--   ELSE Error handling and transaction control handled at usp_SYS_AAA Level'  
    PRINT'-- BEGIN --SYS_AUTHENTICATION_AUTHORIZATION_RESULT = 1 incorrect'
    PRINT'--		EXEC dbo.usp_SYS_SQL_SECURITY_ERROR_HANDLER @HTTP_REFERER,@URL,@BY_WHO,''' + REPLACE(@SPROC_FOR_UPDATES,'''','') + ''''
    PRINT'-- PRINT ''Process attempted to execute outside of NSW System Security Protocol. This error has been logged and will be reviewed by NSW IASO.'''
    PRINT'--    SELECT  ''Process attempted to execute outside of NSW System Security Protocol. This error has been logged and will be reviewed by NSW IASO.'''
    PRINT'-- END --SYS_AUTHENTICATION_AUTHORIZATION_RESULT = 1 incorrect   '
PRINT'END --PROC'
PRINT''
PRINT''
PRINT''
